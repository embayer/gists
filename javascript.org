...
** timestring <-> minutes
#+BEGIN_SRC javascript
/* getMinutesFromTimeString('01:30')
 * // => 90
 */
export function getMinutesFromTimeString (timeString) {
  return moment(timeString, 'HH:mm').diff(moment('00:00', 'HH:mm'), 'minutes')
}
/* getTimeStringFromMinutes(65)
 * // => '01:05'
 */
...
#+END_SRC
*** Find lowest value in array, given an order array
#+BEGIN_SRC javascript
/* findLowest(['c', 'b'], ['a', 'b', 'c'])
 * // => 'b'
 */
function findLowest (arr, order) {
  const idx = arr.reduce((acc, x, idx) => {
    const orderIdx = order.findIndex(y => y === x)
    if (orderIdx < acc.order) acc = {order: orderIdx, arr: idx}
    return acc
  }, {order: arr.length, arr: 0})
  return arr[idx.arr]
}
#+END_SRC
** filter
